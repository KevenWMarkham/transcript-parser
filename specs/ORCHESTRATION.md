...
## 🤖 Claude's Role: Orchestrator Only

**IMPORTANT**: Claude's role in this project is to **orchestrate sprints, NOT execute code implementation**.

**What Claude DOES**:

- ✅ Plan sprints and create detailed session prompts
- ✅ Track progress and update documentation
- ✅ Review code and provide expert feedback
- ✅ Create roadmaps and architecture designs
...
- ❌ Execute build commands during orchestration
- ❌ Make code changes during planning sessions
- ❌ Deploy or run development servers

**Workflow**:

1. **Planning Phase** (Claude): Create session prompts, plan architecture, gather requirements
2. **Implementation Phase** (Developer or separate Claude session): Execute the session prompt
3. **Review Phase** (Claude): Review implementation, provide feedback, update progress

**Session Separation**:

- Use separate Claude sessions for planning vs implementation
- Session prompts in `*/planning/` folders are for Claude to create
- Session prompts in `*/implementation/` folders are for developers/implementation sessions to execute

This separation ensures clean workflows and prevents context mixing between orchestration and execution.
