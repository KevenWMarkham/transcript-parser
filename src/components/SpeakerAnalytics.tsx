...
export function SpeakerAnalytics({ speakers, entries }: SpeakerAnalyticsProps) {
    ...
    <div className="space-y-4">
      ...
      <Card className="p-6 backdrop-blur-sm bg-white/60 border-white/80">
        ...
        <div className="flex flex-wrap gap-3">
          {speakers.map(speaker => (
            <div
              key={speaker.id}
              className={`inline-flex items-center px-4 py-2 rounded-2xl text-sm font-medium border transition-all ${getSpeakerColorClasses(
                speaker.color
              )}`}
            >
              <span
                className={`w-2.5 h-2.5 rounded-full mr-2 ${getDotColorClasses(
                  speaker.color
                )}`}
              ></span>
              {speaker.name}
      ...
      <Card className="p-6 backdrop-blur-sm bg-white/60 border-white/80">
        ...
        <div className="space-y-6">
          {speakerStats.map(({ speaker, segments, totalDuration, percentage }) => (
            <div key={speaker.id} className="space-y-3">
              {/* Speaker header */}
              <div className="flex items-center justify-between">
                <div
                  className={`inline-flex items-center px-4 py-2 rounded-2xl text-sm font-medium border ${getSpeakerColorClasses(
                    speaker.color
                  )}`}
                >
                  <span
